...
class GripService(Node):
	...
	def __init__(self):
		super().__init__('onrobot_2fg7_grip_service')
	...
	def grip_callback(self, request, response):
		try:
			result = self._xmlrpc_proxy.twofg_grip_external(request.id, request.gap, request.force, request.speed)
		except xmlrpc.client.Fault as f:
			self.get_logger().error('Server error: "%s"' % f)
		except Exception as e:
			self.get_logger().error('Error: "%s"' % e)
		self.get_logger().debug('onrobot_2fg7_grip_service request: %s' % request)
		self.get_logger().debug('onrobot_2fg7_grip_service response: %s' % response)
		return response
